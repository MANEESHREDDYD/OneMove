 ...
 * authenticated/anon role and JWT claims, then asserts cross-tenant isolation.
 * Requires service-role + DIRECT_URL in .env.local.
 */
...
function writeRlsStatus(passed) {
  const dir = path.join(process.cwd(), '.next', 'cache', 'onemove');
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(
    path.join(dir, 'rls-status.json'),
    JSON.stringify(
      {
        passed,
...
async function main() {
  ...
  const customerId = profiles.find((user) => user.role === 'customer')?.id;
  const adminId = profiles.find((user) => user.role === 'admin')?.id;
  const partnerId = profiles.find((user) => user.role === 'driver')?.id;
  ...
  const storesWithOrders = new Set((orderRows || []).map((order) => order.merchant_id));
  const ownerToStores = {};

  for (const store of stores || []) {
    (ownerToStores[store.owner_id] ||= []).push(store.id);
  }
  ...
  try {
    ...
    if (custProfiles.length === 1 && custProfiles[0].id === customerId) {
      pass('Customer can read only their own profile.');
    } else {
      fail(`Customer can read ${custProfiles.length} profiles (expected 1 = own).`);
    }
    ...
    const colNames = cardCols.rows.map((row) => row.column_name);
    if (!colNames.includes('phone') && !colNames.includes('email')) {
      pass('safe_profile_cards exposes no phone/email.');
    } else {
      fail('safe_profile_cards exposes sensitive columns: ' + colNames.join(','));
    }

    const custCards = await queryAs(customerId, 'SELECT id FROM safe_profile_cards LIMIT 5;');
    if (custCards.length > 0) {
      pass('Customer can read display names via safe_profile_cards (no PII).');
    } else {
      fail('safe_profile_cards not readable by customer (would break driver name display).');
    }

    const custOrders = await queryAs(customerId, 'SELECT id, customer_id FROM orders;');
    const crossCustomer = custOrders.filter((order) => order.customer_id !== customerId);
    ...
    const ownMerch = merchOrders.filter((order) => merchantStoreIds.has(order.merchant_id));
    const crossMerch = merchOrders.filter(
      (order) => order.merchant_id && !merchantStoreIds.has(order.merchant_id)
    );
    ...
    else fail(`Merchant can read ${crossMerch.length} orders from OTHER merchants.`);

    const merchItems = await queryAs(
      merchantId,
      'SELECT oi.order_id, o.merchant_id FROM order_items oi JOIN orders o ON o.id = oi.order_id;'
    );
    const crossItems = merchItems.filter(
      (row) => row.merchant_id && !merchantStoreIds.has(row.merchant_id)
    );
    ...
    const crossPayments = merchPayments.filter(
      (row) => row.merchant_id && !merchantStoreIds.has(row.merchant_id)
    );
    if (crossPayments.length === 0) pass('Merchant payments are scoped to own orders only.');
    else fail(`Merchant can read payments for ${crossPayments.length} other-merchant orders.`);

    const partnerOrders = await queryAs(partnerId, 'SELECT id, driver_id FROM orders;');
    const otherDriver = partnerOrders.filter((order) => order.driver_id && order.driver_id !== partnerId);
    if (otherDriver.length === 0) pass('Partner sees only assigned/available jobs.');
    ...
    const totalOrders =
      (await adminClient.from('orders').select('*', { count: 'exact', head: true })).count || 0;
    if (adminOrders[0].n >= totalOrders && totalOrders > 0) {
      pass(`Admin can read all ${adminOrders[0].n} orders.`);
    } else {
      fail(`Admin sees ${adminOrders[0].n} of ${totalOrders} orders.`);
    }
