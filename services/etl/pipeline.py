...
def get_data_dirs() -> Dict[str, str]:
    ...
    if not data_root:
        raise ValueError(
            "ZONEPILOT_DATA_ROOT environment variable missing. Real research processing must not use repository data/."
        )
...
def run_etl_pipeline() -> Dict[str, Any]:
    ...
    if not df_raw.empty:
        ...
        valid_mask = df_raw["eta_low_min"].notnull() & df_raw["provenance"].isin(
            ["OBSERVED", "FIXTURE", "SIMULATED", "DERIVED"]
        )
    ...
    print(
        f"Silver Output: {len(df_silver)}, Weather Matches: {weather_matches}, Misses: {weather_misses}, Missingness: {missingness_pct}"
    )
    ...
    r1 = (
        not df_raw.duplicated(subset=["client_event_id"]).any()
        if ("client_event_id" in df_raw and not df_raw.empty)
        else True
    )
    ...
    if not df_raw.empty and "eta_high_min" in df_raw and "eta_low_min" in df_raw:
        ...
        r2 = (
            bool((df_raw.loc[valid_eta, "eta_high_min"] >= df_raw.loc[valid_eta, "eta_low_min"]).all())
            if valid_eta.any()
            else True
        )
    else:
    ...
    r5 = (
        bool(df_raw["protocol"].isin(["ANCHOR", "BURST"]).all())
        if (not df_raw.empty and "protocol" in df_raw)
        else True
    )
...
def build_experiment_a_dataset(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "study_phase" in df and (df["study_phase"] == "DRY_RUN").any():
        ...
        raise ValueError(
            f"CRITICAL: Experiment A dataset builder failed closed. Found {dry_run_count} DRY_RUN rows in dataset."
        )
