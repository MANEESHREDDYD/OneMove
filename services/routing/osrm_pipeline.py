...
def build_osrm_graph():
    ...
    extract_time, _ = measure_time(
        "OSRM Extract", run_docker_osrm, "osrm-extract", "-p", "/opt/car.lua", "/data/pilot_roads.osm.pbf"
    )

    # 2. Partition
    partition_time, _ = measure_time("OSRM Partition", run_docker_osrm, "osrm-partition", "/data/pilot_roads.osrm")

    # 3. Customize
    customize_time, _ = measure_time("OSRM Customize", run_docker_osrm, "osrm-customize", "/data/pilot_roads.osrm")
