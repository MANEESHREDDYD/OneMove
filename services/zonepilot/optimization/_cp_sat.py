...
def _canonical_solution(
    ...
    status, solver = _solve_once(state.model, problem, deadline)
    ...
    selected_open = 0
    for facility_id in sorted(state.open_facility):
    ...
    for scenario_id in sorted(scenario.scenario_id for scenario in problem.scenarios):
        for demand_id in sorted(demand.demand_id for demand in problem.demand_points):
            ...
            if uncovered_value:
                continue
            for facility_id in sorted(facility.facility_id for facility in problem.facilities):
...
def optimize_facilities(problem: OptimizationProblem) -> OptimizationResult:
    ...
    solver, failure = _canonical_solution(problem, state, deadline)
    if failure is not None:
        return failure
    if solver is None:
        raise AssertionError("canonical solve returned neither a solver nor a failure")
    return _optimal_result(problem, state, solver)
