    ...
    def way(self, w):
        ...
        if tags.get("building") == "commercial":
            ...
            self.commercial.append(
                {"id": w.id, "lat": lat, "lon": lon, "h3_index": h3_index, "name": tags.get("name", "Unknown")}
            )

        # Extract Roads
        if "highway" in tags:
            self.roads.append(
                {"id": w.id, "type": tags["highway"], "name": tags.get("name", "Unknown"), "nodes": valid_nodes}
            )
...
def process_pbf(input_pbf, out_dir):
    ...
    print(
        f"Extraction complete. Found {len(extractor.restaurants)} restaurants, {len(extractor.commercial)} commercial buildings, and {len(extractor.roads)} road segments."
    )
