...
def test_point_in_time_weather_leakage_gate():
    ...
    assert selected["id"] == "forecast_A", f"Leakage violation! Selected {selected['id']} issued in future instead of forecast_A"
...
def test_dry_run_exclusion_experiment_a():
    ...
    df_contaminated = pd.DataFrame([
        {"id": "1", "study_phase": "DRY_RUN"},
        {"id": "2", "study_phase": "EXPERIMENT_A"}
    ])
