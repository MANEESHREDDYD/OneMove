...
def test_db_concurrency():
    ...
    for i in range(iterations):
        ...
        for w in range(workers_per_iter):
            ...
            p = ctx.Process(
                target=_worker_task,
                args=(worker_id, barrier, queue, "test_prov", "test_ds", interval, query_hash, db_url),
            )
            p.start()
            processes.append(p)
        ...
        assert running_count == workers_per_iter - 1, (
            f"Iteration {i}: Expected {workers_per_iter - 1} ALREADY_RUNNING, got {running_count}"
        )
        ...
        with psycopg.connect(db_url) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT COUNT(*) FROM zonepilot_ops.collection_runs WHERE logical_interval = %s", (interval,)
                )
