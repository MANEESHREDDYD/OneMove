...
def test_role_attacks():
    ...
    res = requests.post(
        auth_url, json={"email": email, "password": password, "data": {"role": "admin"}}, headers=headers
    )
    ...
    headers_auth = {"apikey": ANON_KEY, "Authorization": f"Bearer {token}", "Content-Type": "application/json"}
    ...
    res = requests.patch(
        f"{SUPABASE_URL}/rest/v1/profiles?id=eq.{user_id}", headers=headers_auth, json={"role": "admin"}
    )
