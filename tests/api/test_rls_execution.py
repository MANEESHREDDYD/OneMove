...
def setup_users():
    ...
    res3 = requests.post(
        f"{SUPABASE_URL}/auth/v1/token?grant_type=password",
        headers=headers,
        json={"email": email3, "password": password},
    )
    ...
    study_res = requests.post(
        f"{SUPABASE_URL}/rest/v1/studies",
    ...
    requests.post(
        ...
        headers=service_headers,
        json={"id": user1["id"], "external_id": "ext1", "hash_key_version": "1"},
    )
    requests.post(
        f"{SUPABASE_URL}/rest/v1/participants",
        headers=service_headers,
        json={"id": user2["id"], "external_id": "ext2", "hash_key_version": "1"},
    )
    requests.post(
        f"{SUPABASE_URL}/rest/v1/participants",
        headers=service_headers,
        json={"id": user3["id"], "external_id": "ext3", "hash_key_version": "1"},
    )

    # 3. Create assignments
    a1_res = requests.post(
    ...
    requests.post(
        f"{SUPABASE_URL}/rest/v1/participant_roles",
        headers=service_headers,
        json={"participant_id": user3["id"], "study_id": study_id, "role": "OWNER"},
    )
...
def test_cross_user_probe_read_rejected(setup_users):
    ...
    res = requests.get(
        f"{SUPABASE_URL}/rest/v1/probe_observations?client_event_id=eq.{client_event_id}", headers=headers
    )
...
def test_update_rejection(setup_users):
    ...
    headers = {
        "apikey": ANON_KEY,
        "Authorization": f"Bearer {setup_users['user1']['token']}",
        "Content-Type": "application/json",
        "Prefer": "return=representation",
    }
    res = requests.patch(
        f"{SUPABASE_URL}/rest/v1/probe_observations?client_event_id=eq.{client_event_id}",
        headers=headers,
        json={"eta_low_min": 50},
    )
...
def test_metadata_owner_escalation_rejected(setup_users):
    ...
    headers = {"apikey": ANON_KEY, "Authorization": f"Bearer {setup_users['user1']['token']}"}
    res = requests.get(
        f"{SUPABASE_URL}/rest/v1/probe_observations?study_id=eq.{setup_users['study_id']}", headers=headers
    )
    assert res.status_code == 200
...
def test_metadata_admin_escalation_rejected(setup_users):
    ...
    res = requests.patch(
        f"{SUPABASE_URL}/rest/v1/profiles?id=eq.{setup_users['user1']['id']}", headers=headers, json={"role": "admin"}
    )
...
def test_current_state_resolution_selects_correction(setup_users):
    ...
    res_view = requests.get(
        f"{SUPABASE_URL}/rest/v1/probe_observations_current?assignment_id=eq.{setup_users['assign1']}", headers=headers
    )
